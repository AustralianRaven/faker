...
const fullEntrypoints = [moduleRegistry, fakerCore.locale];

describe('fakeEval()', () => {
  it('does not allow empty string input', () => {
    expect(() => fakeEval(fakerCore, '')).toThrow(
      new FakerError('Eval expression cannot be empty.')
    );
  });

  it('does not allow empty entrypoints', () => {
    expect(() => fakeEval(fakerCore, 'foobar', [])).toThrow(
  ...
  it('supports patterns after a function call', () => {
    ...
    expect(actual).toBeTypeOf('string');
    expect(
      fakerCore.locale.airline?.airline?.map(({ name }) => name)
  ...
  it('supports patterns after a function reference', () => {
    ...
    expect(actual).toBeTypeOf('string');
    expect(
      fakerCore.locale.airline?.airline?.map(({ iataCode }) => iataCode)
    ).toContain(actual);
  });
  ...
  it('requires a valid expression (missing value)', () => {
    expect(() => fakeEval(fakerCore, 'foo.bar', fullEntrypoints)).toThrow(
  ...
  it('requires a valid expression (trailing dot)', () => {
    ...
    ).toThrow(new FakerError("Found dot without property name in 'airline.'"));
    expect(() =>
      fakeEval(fakerCore, 'airline.airline.()', moduleEntrypoints)
    ).toThrow(
  ...
  it('requires a valid expression (unclosed parenthesis)', () => {
    expect(() =>
      fakeEval(fakerCore, 'airline.airline(', moduleEntrypoints)
    ).toThrow(new FakerError("Missing closing parenthesis in '('"));
    expect(() =>
      fakeEval(fakerCore, 'airline.airline(.iataCode', moduleEntrypoints)
    ).toThrow(new FakerError("Missing closing parenthesis in '(.iataCode'"));
  });
});
